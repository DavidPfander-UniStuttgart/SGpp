...
for set_size in 1000000 10000000 100000000; do
    ...
    for noise in "" "_noise"; do
        ./datadriven/examplesOCL/clustering_cmd --config config_ocl_float_gtx1080ti.cfg --threshold=0.7 --datasetFileName final_paper_datasets/gaussian_c100_size${set_size}_dim10${noise}.arff --level=${level} --lambda=0.00001 --k 5 --write_knn_graph true --scenario_name gaussian_c100_size${set_size}_dim10${noise} > graphs_${set_size}s_${level}l${noise}.log 2>&1
    done
done
