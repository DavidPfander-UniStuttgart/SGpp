...
while nodes <= nodes_end:
    ...
    cmd_raw = "sbatch -N " + str(nodes + 1) + " " + gpu_constraint + " --time 00:30:00 --output=clustering_results/piz_daint_" + str(level) + "l_" + str(dataset_size) +"s_" + str(nodes) + "N.log --error=clustering_results/piz_daint_" + str(level) + "l_" + str(dataset_size) +"s_" + str(nodes) + "N.log_err --wrap=\"srun ./datadriven/examplesMPI/distributed_clustering_cmd --config=" + config_file + " --threshold=" + str(threshold) + " --datasetFileName=" + dataset_file + "  --level=" + str(level) + " --lambda=" + str(lambda_value) + " --verbose_timers=true\""
    print(cmd_raw)
