...
void multdensity(__global const int *starting_points, __global const ulong *dim_zero_flags_v, __global const ulong *level_offsets_v, __global const ulong *level_packed_v, __global const ulong *index_packed_v, const unsigned int non_padding_size, __global const float *alpha, __global float *result,const float lambda, const int startid)
{
...
#define EVAL_BLOCKING 8
	...
	for (size_t j = 0; j < 8; j++) { // ADD
	  ...
	  umid = (umid + fabs(umid));
	  sum += h * (umid);
	  sum *= level_local[(local_index + j) * 40+dim] == decompressed_level ? 1.0f/3.0f : 1.0f; // CHANGE
