...
__kernel __attribute__((reqd_work_group_size(128, 1, 1))) void multdensity(
  ...
  double point_positions_block0[2];
  double point_hs_block0[2];
  double point_hinverses_block0[2];
  ...
  for (int group = 0; group < 1; group++) {
    ...
    barrier(CLK_LOCAL_MEM_FENCE);
    for (int i = 0; i < 128; i++) {
      double zellenintegral = 1.0;
      for (private int dim = 0; dim < 2; dim++) {  // * d -> 14 *d + 2
        double distance = fabs(point_positions_block0[dim] - positions_local[i * 2 + dim]);  // 2
        sum = 1.0 - distance * point_hinverses_block0[dim];                                  // 2
        sum *= hs_local[i * 2 + dim];                                                        // 1
        sum = max(sum, (double)0.0);                                                         // 1
        sum += max((double)(point_hs_block0[dim] * (1.0 - hinverses_local[i * 2 + dim] * distance)),
                   (double)0.0);  // 5
        zellenintegral *=
