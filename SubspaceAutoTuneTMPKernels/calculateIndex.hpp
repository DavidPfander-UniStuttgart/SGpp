...
inline void calculateIndex(bool isModLinear, size_t dim, size_t nextIterationToRecalc,
                           const double *const (&dataTuplePtr)[4],
                           std::vector<uint32_t> &hInversePtr, uint32_t *(&partialIndicesFlat)[4],
                           double *(&partialPhiEvals)[4], uint32_t (&indexFlat)[4],
                           double (&phiEval)[4]) {
  ...
  __m128i indexFlatReg = _mm_set_epi32(
      partialIndicesFlat[3][nextIterationToRecalc], partialIndicesFlat[2][nextIterationToRecalc],
      partialIndicesFlat[1][nextIterationToRecalc], partialIndicesFlat[0][nextIterationToRecalc]);
  ...
  __m256d phiEvalReg = _mm256_set_pd(
      partialPhiEvals[3][nextIterationToRecalc], partialPhiEvals[2][nextIterationToRecalc],
      partialPhiEvals[1][nextIterationToRecalc], partialPhiEvals[0][nextIterationToRecalc]);
    ...
    if (!isModLinear) {
      ...
      for (size_t j = 0; j < 4; j += 1) {
        ...
        if (hInverses.doubleValue[j] == 2) {
          ...
          phi1DEval.doubleValue[j] = 2.0 - hInverses.doubleValue[j] * dataTuple.doubleValue[j];
        } else if (indices.doubleValue[j] == hInverses.doubleValue[j] - 1) {
          phi1DEval.doubleValue[j] =
              hInverses.doubleValue[j] * dataTuple.doubleValue[j] + 1.0 - indices.doubleValue[j];
        } else {
          phi1DEval.doubleValue[j] =
              1.0 -
              fabs(hInverses.doubleValue[j] * dataTuple.doubleValue[j] - indices.doubleValue[j]);
        }
      }
...
inline void calculateIndex2(bool isModLinear, size_t dim, size_t nextIterationToRecalc,
                            // rep
                            const double *const (&dataTuplePtr)[4],
  ...
  __m128i indexFlatReg2 = _mm_set_epi32(
      partialIndicesFlat2[3][nextIterationToRecalc], partialIndicesFlat2[2][nextIterationToRecalc],
      partialIndicesFlat2[1][nextIterationToRecalc], partialIndicesFlat2[0][nextIterationToRecalc]);
  ...
  __m256d phiEvalReg2 = _mm256_set_pd(
      partialPhiEvals2[3][nextIterationToRecalc], partialPhiEvals2[2][nextIterationToRecalc],
      partialPhiEvals2[1][nextIterationToRecalc], partialPhiEvals2[0][nextIterationToRecalc]);
    ...
    if (!isModLinear) {
      ...
      for (size_t j = 0; j < 4; j += 1) {
        if (hInverses.doubleValue[j] == 2) {
          ...
          phi1DEval2.doubleValue[j] = 2.0 - hInverses.doubleValue[j] * dataTuple2.doubleValue[j];
        } else if (indices2.doubleValue[j] == hInverses.doubleValue[j] - 1) {
          phi1DEval2.doubleValue[j] =
              hInverses.doubleValue[j] * dataTuple2.doubleValue[j] + 1.0 - indices2.doubleValue[j];
        } else {
          phi1DEval2.doubleValue[j] =
              1.0 -
              fabs(hInverses.doubleValue[j] * dataTuple2.doubleValue[j] - indices2.doubleValue[j]);
        }
      }
      phiEvalReg = _mm256_mul_pd(phiEvalReg, phi1DEval.doubleRegister);
