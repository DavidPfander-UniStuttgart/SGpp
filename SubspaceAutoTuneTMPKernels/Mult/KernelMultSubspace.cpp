 ...
 * @param chunk_data_start beginning of the range to process
 * @param chunk_data_end end of the range to process
 */
AUTOTUNE_EXPORT sgpp::base::DataVector
KernelMultSubspace(size_t &maxGridPointsOnLevel, bool &isModLinear,
  ...
  sgpp::base::DataVector result(paddedDatasetSize, 0.0);
...
#pragma omp parallel
  {
    ...
    for (size_t i = 0;
         i < (dim + 1) * (SUBSPACEAUTOTUNETMP_PARALLEL_DATA_POINTS +
                          SUBSPACEAUTOTUNETMP_VEC_PADDING);
         i++) {
    ...
    for (size_t curDataStart = chunk_data_start; curDataStart < chunk_data_end;
      ...
      for (size_t subspaceIndex = 0;
           subspaceIndex < allSubspaceNodes.size() - 1; subspaceIndex += 1) {
