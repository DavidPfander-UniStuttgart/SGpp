...
class manager_t {
 ...
 public:
  ...
  cl_kernel build_kernel(const std::string &source, device_t &device,
                         json::node &kernelConfiguration, const std::string &kernel_name,
                         const std::string additional_options = "");

  void configure(bool useConfiguration = false);

  void configure_platform(cl_platform_id platformId, configuration_t &configuration,
...
inline void check(cl_int err, const std::string &message) {
  if (err != CL_SUCCESS) {
    ...
    errorString << message << ", OpenCL error: " << resolve_error(err) << std::endl;
