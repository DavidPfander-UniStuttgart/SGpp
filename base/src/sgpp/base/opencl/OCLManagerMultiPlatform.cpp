...
cl_kernel OCLManagerMultiPlatform::buildKernel(
  ...
  if (verbose) {
    ...
    cl_int err_buildlog = clGetProgramBuildInfo(
        program, device->deviceId, CL_PROGRAM_BUILD_LOG, 0, NULL, &len);
    if (err_buildlog != CL_SUCCESS) {
      std::stringstream errorString;
      errorString << "OCL Error: OpenCL build error. Could not determine size "
                     "of build log. Error code: "
                  << err << std::endl;
      throw sgpp::base::operation_exception(errorString.str());
    }
    std::string buffer(len, '\0');
    err_buildlog = clGetProgramBuildInfo(
        program, device->deviceId, CL_PROGRAM_BUILD_LOG, len, &buffer[0], NULL);
    if (err_buildlog != CL_SUCCESS) {
      std::stringstream errorString;
      errorString << "OCL Error: OpenCL build error. Could not retrieve "
...
void OCLManagerMultiPlatform::configureDevice(
  ...
  if (useConfiguration && devicesNode[deviceName].contains("COUNT") &&
    ...
    std::stringstream errorString;
    errorString << "error: OCLManagerMultiPlatform: \"COUNT\" and \"SELECT\" "
...
void OCLManagerMultiPlatform::set_verbose(bool verbose) {
