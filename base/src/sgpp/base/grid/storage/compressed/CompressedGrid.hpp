...
class compressed_grid {
 ...
 public:
  ...
  compressed_grid(std::vector<int> &gridpoints, size_t dimensions)
      : gridsize(gridpoints.size() / (2 * dimensions)),
        dims(dimensions),
  ...
  compressed_grid(std::vector<G> &level, std::vector<G> &index, size_t dimensions)
      : gridsize(level.size() / (dimensions)),
        dims(dimensions),
  ...
  std::string extraction_ocl_code(std::string dim_zero_flags_name, std::string level_offset_name,
                                  std::string level_packed_name, std::string index_packed_name) {
  ...
  bool check_grid_compression(std::vector<int> &gridpoints) {
    ...
    //   std::cerr << "should be " << gridsize << " is " <<gridpoints.size() / (2 * dims) <<
    ...
    for (size_t i = 0; i < gridsize; i++) {
      ...
      for (size_t d = 0; d < dims; d++) {
        ...
        const T grid_index = static_cast<T>(gridpoints[i * 2 * dims + 2 * d]);
        const T grid_level = static_cast<T>(gridpoints[i * 2 * dims + 2 * d + 1]);
        if (grid_index != extracted_index || grid_level != extracted_level) {
          if (grid_index == 0 && grid_level == 0)  // padding point (0,0) - we cannot compress that
            // anyway - in this case the wrong result is expected. This needs to be handled in the
            // opencl kernels
            continue;
 ...
 private:
  void pack_gridpoint(std::vector<int> &gridpoints, size_t i) {
    std::vector<T> level(dims);
    std::vector<T> index(dims);
    // stage values
    for (int64_t d = 0; d < static_cast<int64_t>(dims); d++) {
  ...
  void pack_level_index_dim(std::vector<T> level, std::vector<T> index, T d, T &dim_zero_flags,
                            T &level_offsets, T &level_packed, T &index_packed) {
    dim_zero_flags <<= 1;
    if (level[d] == 1) {
    ...
    level_offsets >>= 1;
    if (std::is_same<T, uint64_t>::value) level_offsets |= (static_cast<T>(1) << 63);
    if (std::is_same<T, unsigned int>::value) level_offsets |= (static_cast<T>(1) << 31);
  ...
  __attribute__((noinline)) void get_next(T &level_d, T &index_d, T &dim_zero_flags,
                                          T &level_offsets, T &level_packed, T &index_packed) {
    ...
    T level_bits;
    if (std::is_same<T, uint64_t>::value) level_bits = __builtin_clzll(level_offsets) + 1;
    if (std::is_same<T, unsigned int>::value) level_bits = __builtin_clz(level_offsets) + 1;
    level_offsets <<= level_bits;
