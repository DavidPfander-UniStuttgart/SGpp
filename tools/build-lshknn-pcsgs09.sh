...
mkdir -p build
cd build
# -DCMAKE_CUDA_HOST_COMPILER=${LSHKNN_CXX_COMPILER} -DCMAKE_CUDA_COMPILER=${LSHKNN_CMAKE_CUDA_COMPILER}
cmake -DCMAKE_CXX_COMPILER=${LSHKNN_CXX_COMPILER} -DCMAKE_BUILD_TYPE=RelWithDebInfo -DWITH_CUDA=OFF -DWITH_OPENCL=ON  ../
make -j VERBOSE=1
cd ../..
