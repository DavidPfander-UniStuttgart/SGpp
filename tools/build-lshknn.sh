...
mkdir -p build
cd build
cmake -DCMAKE_CXX_COMPILER=${LSHKNN_CXX_COMPILER} -DCMAKE_CUDA_HOST_COMPILER=${LSHKNN_CXX_COMPILER} -DCMAKE_CUDA_COMPILER=${LSHKNN_CMAKE_CUDA_COMPILER} -DCMAKE_BUILD_TYPE=RelWithDebInfo  ../
make -j VERBOSE=1
cd ../..
