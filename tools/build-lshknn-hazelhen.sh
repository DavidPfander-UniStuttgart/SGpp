...
mkdir -p build
cd build
cmake -DCMAKE_CXX_COMPILER=${LSHKNN_CXX_COMPILER} -DCMAKE_CUDA_HOST_COMPILER=${LSHKNN_CXX_COMPILER} -DCMAKE_BUILD_TYPE=RelWithDebInfo -DWITH_CUDA=OFF -DWITH_OPENCL=ON  ../
make -j VERBOSE=1
cd ../..
