...
for level in levels:
    for dim in range(2, 11, 2):
        ...
        f_result = open("results/results_gaussian_c" + str(clusters) + "_l" + str(level) + "_d" + str(dim) + "_" + deviceName + "_" + precision + ".csv", "w")
        ...
        for dataset_size in chain(range(20000, 110000, 20000), range(200000, 1100000, 200000)):
           ...
           cmd = "./datadriven/examplesOCL/clustering_cmd --datasetFileName " + dataset_file_name + " --level " + str(level) + " --lambda " + str(lambdas) + " --threshold " + str(threshold) + " --k " + str(k) + " --config " + configFile + " --refinement_steps " + str(refinement_steps) + " --refinement_points " + str(refinement_points) + " --coarsen_points " + str(coarsen_points) + " --coarsen_threshold " + str(coarsen_threshold)
           print("cmd: " + cmd)
