...
class dataset_configuration:
    def __init__(self, num_clusters, dataset_size, dim, noise_suffix):
...
class parameter_tuner:
    ...
    def tune_lambda(self, l_config, t_config, d_config, level, k, epsilon):
        ...
        while lambda_iteration < l_config.lambda_iterations:
            ...
            for lambda_value in lambda_values:
                ...
                best_ARI, best_threshold = self.tune_threshold(l_config, t_config, d_config, level, k, epsilon, lambda_value)
    ...
    def tune_threshold(self, l_config, t_config, d_config, level, k, epsilon, lambda_value):
        ...
        while threshold_step > t_config.threshold_step_min:
            ...
            for threshold in thresholds:
                ...
                cmd = cmd_pattern.format(num_tasks=self.num_tasks, config=self.config, mpi_config=self.mpi_config, num_clusters=d_config.num_clusters, dataset_size=d_config.dataset_size, dim=d_config.dim, noise_suffix=d_config.noise_suffix, level=level, lambda_value=lambda_value, k=k, epsilon=epsilon, threshold=threshold)
                ...
                results_file = "tune_gaussian_c{num_clusters}_size{dataset_size}_dim{dim}{noise_suffix}_cluster_map.csv".format(num_clusters=d_config.num_clusters, dataset_size=d_config.dataset_size, dim=d_config.dim, noise_suffix=d_config.noise_suffix)
                reference_file = "../../DissertationCodeTesla1/SGpp/datasets_WPDM18/gaussian_c{num_clusters}_size{dataset_size}_dim{dim}{noise_suffix}_class.arff".format(num_clusters=d_config.num_clusters, dataset_size=d_config.dataset_size, dim=d_config.dim, noise_suffix=d_config.noise_suffix)
    ...
    def tune(self, t_config, l_config, d_config, level, k, epsilon):
        ...
        self.tune_lambda(l_config, t_config, d_config, level, k, epsilon)
...
# dataset_size num_clusters level
experiments = [(int(1E6), 10, 6), (int(1E6), 100, 7), (int(1E7), 10, 7), (int(1E7), 100, 7)]
for dataset_size, num_clusters, level in experiments:
    #############################
    ...
    # configure dataset
    #############################
    d_config = dataset_configuration(num_clusters=num_clusters, dataset_size=dataset_size, dim=10, noise_suffix="_noise")

    t = parameter_tuner()
    t.tune(t_config, l_config, d_config, level, k, epsilon)
