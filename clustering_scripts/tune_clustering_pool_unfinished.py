...
# def evaluate(cmd, log_file_name, is_first_lambda, is_first_overall):
def evaluate(args):
    cmd, log_file_name, is_first_lambda, is_first_overall, hostname = args
    if hostname == 'argon-gtx':
        current = mp.current_process()
        process_num = (current._identity[0] - 1) % 8
        print("process_num:", process_num)
        cmd = cmd.format(device_num=process_num)

    # if not is_first_overall:
    #     # copy kNN result
    #     os.system("cp {} {}")
    # if not is_first_lambda:
    #     os.system("cp {} {}".format("tune_gaussian_c10_size1000000_dim10_noise_graph.csv"))
...
class parameter_tuner:
    ...
    def __init__(self):
        ...
        self.num_tasks=9
        self.config_map={"argon-gtx": "OCL_configs/config_ocl_float_gtx1080ti_device{device_num}.cfg", "argon-tesla2": "OCL_configs/config_ocl_float_QuadroGP100.cfg", "argon-tesla1": "OCL_configs/config_ocl_float_P100.cfg", "pcsgs09": "OCL_configs/config_ocl_float_i76700k.cfg"}
    ...
    def get_cmd_pattern(self, threshold_worker = False):
    ...
    def tune_threshold(self, l_config, t_config, d_config, dataset_size, level, k, epsilon, lambda_value):
        ...
        while threshold_step > t_config.threshold_step_min:
            ...
            self.f_log.flush()
            # first collect work, then schedule, then evalaute
            pool_work = []
            for threshold in thresholds:
                ...
                # detected_clusters, datapoints_clusters, duration, iterations = evaluate(cmd, self.f_log, self.is_first_lambda, self.is_first_overall, mutex)
                pool_work += [[cmd, self.log_file_name, self.is_first_lambda, self.is_first_overall, self.hostname]]
                if self.is_first_overall:
            ...
            self.f_log.close()
            pool_results = []
            if threshold_iteration == 0:
                # first element only!
                # generates file to reuse (density estimation and kNN)
                print("first task:", pool_work[0:1])
                pool_results += self.pool.map(evaluate, pool_work[0:1])
                # execute remaining work in parallel
                print("other tasks:", pool_work[1:])
                pool_results += self.pool.map(evaluate, pool_work[1:])
            else:
                pool_results += self.pool.map(evaluate, pool_work)
            print(pool_results)

            self.f_log = open(self.log_file_name, "a")

            for detected_clusters, datapoints_clusters, duration, iterations in pool_results:
    ...
    def tune(self, t_config, l_config, d_config, k, epsilon):
        for dataset_size in d_config.dataset_sizes:
            ...
            self.log_file_name = "tune_clustering_" + str(dataset_size) + "s_" + str(d_config.num_clusters) + "c" + str(d_config.noise_suffix) + "_" + str(level) + "l" + ".log"
            print("log_file_name:", self.log_file_name)
            self.f_log = open(self.log_file_name, "w")
...
t_config = threshold_configuration(threshold_intervals = 8.0, threshold_start = 0.0, threshold_stop = 2500.0, threshold_step_min = 50.0)
