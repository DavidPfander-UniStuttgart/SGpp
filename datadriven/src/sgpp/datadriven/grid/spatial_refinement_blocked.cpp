...
void spatial_refinement_blocked::neighbor_pos_dir_blocked(std::vector<int64_t> &schedule_level,
...
void spatial_refinement_blocked::verify_support_blocked() {
  int64_t num_candidates = static_cast<int64_t>(schedule_level.size() / dim);
...
void spatial_refinement_blocked::refine_impl() {
...
spatial_refinement_blocked::spatial_refinement_blocked(int64_t dim, int64_t max_level,
                                                       int64_t min_support,
...
void spatial_refinement_blocked::refine() {
  levels.clear();
  indices.clear();
...
std::vector<int64_t> &spatial_refinement_blocked::get_levels() { return levels; }
std::vector<int64_t> &spatial_refinement_blocked::get_indices() { return indices; }

void spatial_refinement_blocked::print_level(std::vector<int64_t> l) {
  std::cout << "l: (";
  for (int64_t d = 0; d < dim; d += 1) {
    if (d > 0) {
      std::cout << ", ";
...
void spatial_refinement_blocked::print_index(std::vector<int64_t> i) {
  std::cout << "i: (";
  for (int64_t d = 0; d < dim; d += 1) {
    if (d > 0) {
      std::cout << ", ";
...
void spatial_refinement_blocked::print_pos(std::vector<double> pos) {
  std::cout << "pos: (";
  for (int64_t d = 0; d < dim; d += 1) {
    if (d > 0) {
      std::cout << ", ";
...
void spatial_refinement_blocked::write_grid_positions(int64_t dim,
