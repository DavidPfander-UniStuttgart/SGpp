__kernel void verify_support(__global double *data, const long data_size,
                             __global long *schedule_level, __global long *schedule_index,
                             __global unsigned char *schedule_support, long min_support) {
  ...
  for (long data_index = 0; data_index < data_size; data_index += 16) {
    ...
    for (int local_data_index = 0; local_data_index < 16; local_data_index += 1) {
      bool has_support = true;
      for (int d = 0; d < DIM; d += 1) {
        // if (data[d * data_size + data_index] < bound_left[d] ||
        //     data[d * data_size + data_index] > bound_right[d]) {
