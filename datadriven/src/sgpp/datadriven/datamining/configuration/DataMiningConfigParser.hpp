...
class DataMiningConfigParser {
 public:
  ...
  std::vector<int64_t> parseIntArray(json::dict_node &dict, const std::string &key,
 ...
 private:
  ...
  std::string parseString(json::dict_node &dict, const std::string &key,
                          const std::string &defaultValue, const std::string &parentNode) const;
  double parseDouble(json::dict_node &dict, const std::string &key, double defaultValue,
                     const std::string &parentNode) const;
  size_t parseUInt(json::dict_node &dict, const std::string &key, size_t defaultValue,
                   const std::string &parentNode) const;
  int64_t parseInt(json::dict_node &dict, const std::string &key, int64_t defaultValue,
                   const std::string &parentNode) const;
  bool parseBool(json::dict_node &dict, const std::string &key, bool defaultValue,
                 const std::string &parentNode) const;

  void parseSLESolverConfig(json::dict_node &dict, SLESolverConfiguration &config,
                            const SLESolverConfiguration &defaults,
                            const std::string &parentNode) const;

  void parseDataTransformationConfig(json::dict_node &dict, DataTransformationConfig &config,
                                     const DataTransformationConfig &defaults,
                                     const std::string &parentNode) const;
  void parseRosenblattTransformationConfig(json::dict_node &dict,
