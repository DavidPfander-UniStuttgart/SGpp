...
#pragma omp barrier
  ...
  size_t
      validIndices[X86COMBINED_PARALLEL_DATA_POINTS + X86COMBINED_VEC_PADDING];
  size_t validIndicesCount;

  size_t
      levelIndices[X86COMBINED_PARALLEL_DATA_POINTS + X86COMBINED_VEC_PADDING];
  ...
  for (size_t dataIndexBase = start_index_data; dataIndexBase < end_index_data;
    ...
    for (size_t subspaceIndex = 0; subspaceIndex < subspaceCount;
      ...
      for (size_t parallelIndex = 0;
           parallelIndex < X86COMBINED_PARALLEL_DATA_POINTS; parallelIndex++) {
      ...
      // width
      size_t paddingSize =
          std::min((int)(validIndicesCount + X86COMBINED_VEC_PADDING),
                   X86COMBINED_PARALLEL_DATA_POINTS + X86COMBINED_VEC_PADDING);
      ...
      if (subspace.type == SubspaceNodeCombined::SubspaceType::ARRAY) {
        ...
        listMultInner(dim, alpha, dataIndexBase, end_index_data, subspace,
                      subspace.subspaceArray.data(), validIndicesCount,
                      validIndices, levelIndices, evalIndexValuesAll,
                      intermediatesAll);
        ...
        listMultInner(dim, alpha, dataIndexBase, end_index_data, subspace,
                      listSubspace, validIndicesCount, validIndices,
