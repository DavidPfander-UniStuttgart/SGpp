...
base::OperationMultipleEval *createStreamingModOCLUnifiedConfigured(
  ...
  if ((*parameters)["INTERNAL_PRECISION"].get() == "float") {
    return new datadriven::OperationMultiEvalStreamingModOCLUnified<float>(
        grid, dataset, manager, parameters, isModLinear);
  } else if ((*parameters)["INTERNAL_PRECISION"].get() == "double") {
    return new datadriven::OperationMultiEvalStreamingModOCLUnified<double>(
        grid, dataset, manager, parameters, isModLinear);
  } else {
