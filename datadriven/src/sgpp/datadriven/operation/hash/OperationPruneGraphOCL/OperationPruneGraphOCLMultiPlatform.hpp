...
class OperationPruneGraphOCLMultiPlatform : public OperationPruneGraphOCL {
 private:
  ...
  std::shared_ptr<sgpp::datadriven::DensityOCLMultiPlatform::KernelPruneGraph<T>> graph_kernel;
 ...
 public:
  ...
  OperationPruneGraphOCLMultiPlatform(
    ...
    for (size_t i = 0; i < gridSize; i++) alphaVector[i] = static_cast<T>(alpha[i]);
  ...
  OperationPruneGraphOCLMultiPlatform(
    ...
    if (verbose)
      std::cout << "Grid stored into integer array! Number of gridpoints: " << gridSize
                << std::endl;
    for (size_t i = 0; i < gridSize; i++) alphaVector[i] = static_cast<T>(alpha[i]);
    double *data_raw = data.getPointer();
    for (size_t i = 0; i < data.getSize(); i++) dataVector[i] = static_cast<T>(data_raw[i]);
    if (verbose)
      std::cout << "Data stored into float array! Number of datapoints: " << data.getSize()
                << std::endl;
