...
class GraphCreationWorker : public MPIWorkerGraphBase {
 protected:
  ...
  void divide_workpackages(int *package, std::vector<int> &graph) {
    ...
    int packagesize =
        static_cast<int>(MPIEnviroment::get_configuration()["PREFERED_PACKAGESIZE"].getInt());
    int *partial_result = new int[package[1] * k];
    SimpleQueue<int> workitem_queue(package[0], package[1], packagesize, sub_worker_comm,
