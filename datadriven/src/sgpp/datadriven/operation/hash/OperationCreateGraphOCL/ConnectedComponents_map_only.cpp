...
// TODO: rename graph requires std::set?
void connected_components_deferred(std::vector<int64_t> &directed, std::vector<int64_t> &map,
                                   const int64_t N, const int64_t k, const size_t start_index,
                                   std::vector<std::vector<int64_t>> &rename_graph) {
...
void connected_components_deferred(std::vector<int64_t> &directed, const int64_t k,
  ...
  for (size_t i = 0; i < N; i += 1) {
    if (map[i] == 0) {
      detail::connected_components_deferred(directed, map, N, k, i, rename_graph);
    }
  }
  std::cout << "num clusters: " << detail::global_cluster_id - 1 << std::endl;
}
