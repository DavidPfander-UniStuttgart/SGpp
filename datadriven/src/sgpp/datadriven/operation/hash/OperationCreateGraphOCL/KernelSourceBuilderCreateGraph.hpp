...
class SourceBuilderCreateGraph : public base::KernelSourceBuilderBase<real_type> {
      ...
      for (size_t i = 1; i < k; i++) {
        if (use_select) {
          output << this->indent[2] << "maxindex = select(maxindex, " << i
                 << ", k_dists[maxindex] < k_dists[" << i << "]);" << std::endl;
        } else {
 ...
 public:
  ...
  std::string generateSource(size_t dimensions, size_t k, size_t dataSize, size_t problemsize) {
    ...
    if (!use_approx) sourceStream << init_k_registers(k) << std::endl;
    ...
    if (use_approx) {
      sourceStream << this->indent[0] << "__local " << this->floatType() << " data_local["
                   ...
                   << this->indent[0] << "__private int  index_reg[" << approxRegCount << "];"
                   << std::endl
                   << this->indent[1] << "for (int i = 0 ; i < " << approxRegCount << "; i++) "
                   << std::endl
      ...
      if (localWorkgroupSize != approxRegCount) sourceStream << this->indent[1] << "}" << std::endl;
      ...
      if (localWorkgroupSize == approxRegCount) {
        sourceStream << this->indent[1] << "neighbors[chunk_index * " << k
                     << " + neighbor] = maxindex + index_reg[maxindex] * " << localWorkgroupSize
                     << ";" << std::endl;
      } else {
        sourceStream << this->indent[1] << "neighbors[chunk_index * " << k
      ...
      sourceStream << this->indent[0] << "__local " << this->floatType() << " data_local["
                   ...
                   << find_max_index(k, true) << this->indent[2]
                   << "if (dist < k_dists[maxindex] && i + group * " << localWorkgroupSize
      ...
      sourceStream << this->indent[0] << "for (int i = 0; i <    " << dataSize << "; i++) {"
                   ...
                   << find_max_index(k, true) << this->indent[2]
                   << "if (dist < k_dists[maxindex]) {" << std::endl
