...
AUTOTUNE_KERNEL(
    sgpp::base::DataVector(size_t &, bool &, sgpp::base::DataMatrix &, size_t,
...
AUTOTUNE_KERNEL(sgpp::base::DataVector(
                    size_t &, bool &, sgpp::base::DataMatrix &, size_t &, sgpp::base::GridStorage &,
                    std::vector<sgpp::datadriven::SubspaceAutoTuneTMP::SubspaceNode> &,
...
OperationMultipleEvalSubspaceAutoTuneTMP::OperationMultipleEvalSubspaceAutoTuneTMP(
    ...
    : base::OperationMultipleEval(grid, dataset),
      ...
      write_stats(false),
      listRatio(0.2),
      parallelDataPointsMult(256),
      parallelDataPointsMultTrans(256),
      vectorPaddingMult(4),
      vectorPaddingMultTrans(4),
...
void OperationMultipleEvalSubspaceAutoTuneTMP::padDataset(sgpp::base::DataMatrix &dataset) {
  size_t chunkSize = std::max(parallelDataPointsMult, parallelDataPointsMultTrans);
...
void OperationMultipleEvalSubspaceAutoTuneTMP::mult(sgpp::base::DataVector &alpha,
                                                    sgpp::base::DataVector &result) {
  if (!autotune::KernelMultSubspace.is_compiled() || configuration_changed_mult) {
    ...
    if (configuration.contains("SubspaceMult")) {
      ...
      parallelDataPointsMult = std::stoll(parameters["SUBSPACEAUTOTUNETMP_PARALLEL_DATA_POINTS"]);
      vectorPaddingMult = std::stoll(parameters["SUBSPACEAUTOTUNETMP_VEC_PADDING"]);
      this->padDataset(dataset);
    } else {
      ...
      parameters["SUBSPACEAUTOTUNETMP_VEC_PADDING"s] = std::to_string(vectorPaddingMult);
  ...
  result = autotune::KernelMultSubspace(maxGridPointsOnLevel, isModLinear, paddedDataset,
...
void OperationMultipleEvalSubspaceAutoTuneTMP::multTranspose(sgpp::base::DataVector &source,
                                                             sgpp::base::DataVector &result) {
  if (!autotune::KernelMultTransposeSubspace.is_compiled() || configuration_changed_multTrans) {
    std::string sgpp_base_include;
    std::string boost_include;
    std::string autotunetmp_include;
    std::string vc_include;
    ...
    if (configuration.contains("SubspaceMultTrans")) {
      ...
      vectorPaddingMultTrans = std::stoll(parameters["SUBSPACEAUTOTUNETMP_VEC_PADDING"]);
      ...
      parameters["SUBSPACEAUTOTUNETMP_ENABLE_SUBSPACE_SKIPPING"s] = "1"s;
      parameters["SUBSPACEAUTOTUNETMP_UNROLL"s] = "0"s;
      parameters["SUBSPACEAUTOTUNETMP_VEC_PADDING"s] = std::to_string(vectorPaddingMultTrans);
      parameters["SUBSPACEAUTOTUNETMP_LIST_RATIO"s] = std::to_string(listRatio);
      parameters["SUBSPACEAUTOTUNETMP_ENABLE_PARTIAL_RESULT_REUSAGE"s] = "1"s;
    }
  ...
  result = autotune::KernelMultTransposeSubspace(maxGridPointsOnLevel, isModLinear, paddedDataset,
                                                 paddedDatasetSize, storage, allSubspaceNodes,
                                                 allLevelsIndexMap, maxLevel, source);

  source.resize(originalSourceSize);
  this->duration = this->timer.stop();
}

void OperationMultipleEvalSubspaceAutoTuneTMP::set_write_stats(const std::string &stats_file_name,
...
double OperationMultipleEvalSubspaceAutoTuneTMP::getDuration() { return this->duration; }

void OperationMultipleEvalSubspaceAutoTuneTMP::tune_mult(sgpp::base::DataVector &alpha,
                                                         sgpp::base::DataVector &result,
  ...
  std::function<bool(sgpp::base::DataVector)> test_result =
    ...
    for (size_t i = 0; i < result_compare.size(); i++) {
      ...
      if (diff >= 1E-5) {
        std::cout << "mine: " << result[i] << " ref: " << result_compare[i] << " diff: " << diff
                  << std::endl;
        return false;
      }
    }
  ...
  if (tuner_name.compare("bruteforce") == 0) {
    autotune::tuners::bruteforce tuner(autotune::KernelMultSubspace, parameters);
    tuner.set_parameter_adjustment_functor([](auto &parameters) -> void {
      autotune::fixed_set_parameter<int64_t> &p_unroll =
          parameters.template get_by_name<autotune::fixed_set_parameter<int64_t>>(
    ...
    optimal_parameters = tuner.tune(maxGridPointsOnLevel, isModLinear, paddedDataset,
                                    paddedDatasetSize, allSubspaceNodes, alpha);
  } else if (tuner_name.compare("line_search") == 0) {
    autotune::tuners::line_search tuner(autotune::KernelMultSubspace, parameters,
                                        10 * parameters.size());
    tuner.set_parameter_adjustment_functor([](auto &parameters) -> void {
      autotune::fixed_set_parameter<int64_t> &p_unroll =
          parameters.template get_by_name<autotune::fixed_set_parameter<int64_t>>(
    ...
    autotune::tuners::neighborhood_search tuner(autotune::KernelMultSubspace, parameters, 100);
    tuner.set_parameter_adjustment_functor([](auto &parameters) -> void {
      autotune::fixed_set_parameter<int64_t> &p_unroll =
          parameters.template get_by_name<autotune::fixed_set_parameter<int64_t>>(
              "SUBSPACEAUTOTUNETMP_UNROLL");
    ...
    autotune::tuners::monte_carlo tuner(autotune::KernelMultSubspace, parameters_randomizable, 100);
    tuner.set_parameter_adjustment_functor([](auto &parameters) -> void {
      autotune::fixed_set_parameter<int64_t> &p_unroll =
          parameters.template get_by_name<autotune::fixed_set_parameter<int64_t>>(
              "SUBSPACEAUTOTUNETMP_UNROLL");
    ...
    autotune::randomizable_set optimal_parameters_randomizable =
        tuner.tune(maxGridPointsOnLevel, isModLinear, paddedDataset, paddedDatasetSize,
                   allSubspaceNodes, alpha);
    autotune::KernelMultSubspace.set_parameter_values(optimal_parameters_randomizable);
...
void OperationMultipleEvalSubspaceAutoTuneTMP::tune_multTranspose(sgpp::base::DataVector &source,
                                                                  sgpp::base::DataVector &result,
                                                                  const std::string &scenario_name,
                                                                  const std::string &tuner_name,
                                                                  uint32_t repetitions) {
  ...
  autotune::KernelMultTransposeSubspace.set_precompile_validate_parameter_functor(
      [this, &source, &result](autotune::parameter_value_set &parameters) -> bool {
        if (std::stoll(parameters["SUBSPACEAUTOTUNETMP_UNROLL"]) == 1 &&
            std::stoull(parameters["SUBSPACEAUTOTUNETMP_VEC_PADDING"]) !=
                2 * this->AVX_vector_width) {
          return false;
  ...
  if (tuner_name.compare("bruteforce") == 0) {
    autotune::tuners::bruteforce tuner(autotune::KernelMultTransposeSubspace, parameters);
    tuner.set_parameter_adjustment_functor([](auto &parameters) -> void {
      autotune::fixed_set_parameter<int64_t> &p_unroll =
          parameters.template get_by_name<autotune::fixed_set_parameter<int64_t>>(
              "SUBSPACEAUTOTUNETMP_UNROLL");
    ...
    tuner.setup_test(test_result);
    optimal_parameters =
        tuner.tune(maxGridPointsOnLevel, isModLinear, paddedDataset, paddedDatasetSize, storage,
                   allSubspaceNodes, allLevelsIndexMap, maxLevel, source);
  } else if (tuner_name.compare("line_search") == 0) {
    autotune::tuners::line_search tuner(autotune::KernelMultTransposeSubspace, parameters,
                                        10 * parameters.size());
    tuner.set_parameter_adjustment_functor([](auto &parameters) -> void {
      autotune::fixed_set_parameter<int64_t> &p_unroll =
          parameters.template get_by_name<autotune::fixed_set_parameter<int64_t>>(
    ...
    autotune::tuners::neighborhood_search tuner(autotune::KernelMultTransposeSubspace, parameters,
                                                100);
    tuner.set_parameter_adjustment_functor([](auto &parameters) -> void {
      autotune::fixed_set_parameter<int64_t> &p_unroll =
          parameters.template get_by_name<autotune::fixed_set_parameter<int64_t>>(
    ...
    autotune::randomizable_set optimal_parameters_randomizable =
        tuner.tune(maxGridPointsOnLevel, isModLinear, paddedDataset, paddedDatasetSize, storage,
                   allSubspaceNodes, allLevelsIndexMap, maxLevel, source);
    autotune::KernelMultTransposeSubspace.set_parameter_values(optimal_parameters_randomizable);
  ...
  if (tuner_name.compare("monte_carlo") != 0) {
    // setup optimal parameters for next call to mult
    autotune::KernelMultTransposeSubspace.set_parameter_values(optimal_parameters);
    autotune::KernelMultTransposeSubspace.create_parameter_file();
    autotune::KernelMultTransposeSubspace.compile();
