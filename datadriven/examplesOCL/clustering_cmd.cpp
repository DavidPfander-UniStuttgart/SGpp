...
int main(int argc, char **argv) {
  ...
  description.add_options()("help", "display help")(
      ...
      "coarsening")("weight_support_coarsening_threshold",
                    boost::program_options::value<double>(&weight_support_coarsening_threshold)
                        ->default_value(0.0),
                    "for density estimation, prune if per grid points sum is <= given "
                    "threshold")(
      "use_support_refinement", boost::program_options::bool_switch(&use_support_refinement),
      "use support refinement to guess an initial grid without using the CG "
      "solver")(
      "support_refinement_min_support",
      boost::program_options::value<int64_t>(&support_refinement_min_support)->default_value(1),
  ...
  if (reuse_density_grid.compare("") != 0) {
    ...
    if (!use_support_refinement) {
      ...
      std::cout << "support_refinement_min_support:" << support_refinement_min_support << std::endl;
    ...
    for (size_t i = 0; i < refinement_steps; i++) {
      ...
      if (refinement_points > 0 && i > 0) {
        ...
        std::cout << "surplus refinement: new grid size " << grid->getSize() << std::endl;
      }
      ...
      if (weight_support_coarsening && i == 0) {
        ...
        std::cout << std::endl;
        size_t old_size = alpha.getSize();
        sgpp::base::WeightSupportCoarseningFunctor weight_support_coarsen_functor(
            b, weight_support_coarsening_threshold);
        grid_generator.coarsen(weight_support_coarsen_functor, b);
        alpha.resize(grid->getSize());
        std::cout << "b-based coarsening: removed " << (old_size - grid->getSize())
                  << " grid points, remaining: " << grid->getSize() << std::endl;
