...
int main(int argc, char **argv) {
  ...
  description.add_options()("help", "display help")(
      ...
      "scenario_name", boost::program_options::value<std::string>(&scenario_name),
      ...
      "write_knn_graph",
      boost::program_options::value<bool>(&write_knn_graph)->default_value(false),
      "write the knn graph calculated to a csv-file")(
      "cluster_file", boost::program_options::value<std::string>(&cluster_file)->default_value(""),
      ...
      "will be written.")("knn_file", boost::program_options::value<std::string>(&knn_filename),
                          "Filename for the knn graph")(
      "pruned_knn_file", boost::program_options::value<std::string>(&pruned_knn_filename),
      "Filename for the pruned knn graph")(
      "write_pruned_knn_graph",
      boost::program_options::value<bool>(&write_pruned_knn_graph)->default_value(false),
      "write the pruned knn graph calculated to a csv-file")(
      "write_cluster_map",
      boost::program_options::value<bool>(&write_cluster_map)->default_value(false),
      "write mapped clusters to a csv-file")(
      "write_density_grid",
      boost::program_options::value<bool>(&write_density_grid)->default_value(false),
      "write the coordinates, levels and indices to a csv-file")(
      "write_evaluated_density_full_grid",
      boost::program_options::value<bool>(&write_evaluated_density_full_grid)->default_value(false),
      "evaluate density function on full grid and write result to a "
      "csv-file")("record_timings",
                  boost::program_options::value<bool>(&record_timings)->default_value(false),
  ...
  if (write_density_grid) {
    std::ofstream out_grid(std::string("results/") + scenario_name + "_grid_coord.csv");
    ...
    out_grid = std::ofstream(std::string("results/") + scenario_name + "_grid_levels.csv");
    ...
    out_grid.close();
    out_grid = std::ofstream(std::string("results/") + scenario_name + "_grid_indices.csv");
    for (size_t j = 0; j < grid->getSize(); j++) {
      sgpp::base::GridPoint point = storage.getPoint(j);
      sgpp::base::GridPoint::level_type l;
      sgpp::base::GridPoint::index_type i;
...

  {
    ...
    if (write_knn_graph) {
      knn_op.write_graph_file(std::string("results/") + scenario_name + "_graph_naive.csv", graph,
                              k);
    }
...

  {
    ...
    if (write_pruned_knn_graph) {
      std::vector<int64_t> graph_converted;
      graph_converted = std::vector<int64_t>(graph.begin(), graph.end());
      knn_op.write_graph_file(std::string("results/") + scenario_name + "_graph_pruned.csv",
                              graph_converted, k);
    }
