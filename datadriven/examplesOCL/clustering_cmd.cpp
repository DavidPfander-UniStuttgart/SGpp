...
class DensityBCoarseningFunctor : public CoarseningFunctor {
 ...
 public:
   ...
   */
  DensityBCoarseningFunctor(DataVector &b, double threshold = 0.0) : b(b), threshold(threshold) {}
...
double testDistanceAccuracy(const std::vector<double> data, const std::vector<int> correct,
                            const std::vector<int> result, const int size, const int dim,
                            const int k) {
  ...
  for (int i = 0; i < size; ++i) {
    for (int j = 0; j < k; ++j) {
      ...
      for (int d = 0; d < dim; ++d) {
        dist_correct += (data[size * d + i] - data[size * d + correct[k * i + j]]) *
...
int main(int argc, char **argv) {
  ...
  description.add_options()("help", "display help")(
      ...
      "reuse_knn_graph", boost::program_options::value<std::string>(&reuse_knn_graph),
      "name of a file containing the knn graph of the dataset; knn graph is "
      "not computed if this option is set")(
      "reuse_density_grid", boost::program_options::value<std::string>(&reuse_density_grid),
      ...
      "liblshknn)")("lsh_tables",
                    boost::program_options::value<uint64_t>(&lsh_tables)->default_value(50),
                    "number of hash tables for lsh knn")(
      "lsh_hashes", boost::program_options::value<uint64_t>(&lsh_hashes)->default_value(15),
      "number of hash functions used by lsh knn")(
      "lsh_w", boost::program_options::value<double>(&lsh_w)->default_value(1.5),
      ...
      "write knn graph to file")("write_pruned_knn_graph",
                                 boost::program_options::bool_switch(&write_pruned_knn_graph),
                                 "write the pruned knn graph calculated to a csv-file")(
      "write_cluster_map", boost::program_options::bool_switch(&write_cluster_map),
      "write mapped clusters to a csv-file")(
      "write_density_grid", boost::program_options::bool_switch(&write_density_grid),
      ...
      "csv-file")("record_timings", boost::program_options::bool_switch(&record_timings),
      ...
      boost::program_options::value<uint64_t>(&sampling_chunk_count)->default_value(1),
      "number of chunks for sampling of dataset for knn")(
      "target_clusters", boost::program_options::value<int64_t>(&target_clusters)->default_value(0),
      ...
      boost::program_options::value<double>(&b_coarsening_threshold)->default_value(0.0),
      "for density estimation, prune if per grid points sum is <= given "
      "threshold")(
      "use_datadriven_refinement", boost::program_options::bool_switch(&use_datadriven_refinement),
      "use datadriven refinement to guess an initial grid without using the CG solver")(
      "datadriven_refinement_min_support",
      boost::program_options::value<int64_t>(&datadriven_refinement_min_support)->default_value(1),
      "for datadriven refinement, minimal number of data points on support for accepting data "
      "point")
      // (
  ...
  std::cout
      ...
      << static_cast<std::chrono::duration<double>>(loading_data_end - loading_data_start).count()
      << std::endl;
  ...
  if (reuse_density_grid.compare("") != 0) {
    ...
    std::chrono::time_point<std::chrono::system_clock> reuse_grid_start, reuse_grid_stop;
    ...
    std::chrono::duration<double> reuse_grid_seconds = reuse_grid_stop - reuse_grid_start;
    std::cout << "loading density grid (reuse) duration: " << reuse_grid_seconds.count() << "s"
              << std::endl;
    ...
    std::chrono::time_point<std::chrono::system_clock> grid_create_start, grid_create_stop;
    grid_create_start = std::chrono::system_clock::now();
    if (!use_datadriven_refinement) {
      grid = std::unique_ptr<base::Grid>(base::Grid::createLinearGrid(dimension));
      base::GridGenerator &grid_generator = grid->getGenerator();
      grid_generator.regular(level);
      std::cout << "initial grid created, grid points: " << grid->getSize() << std::endl;
    } else {
      sgpp::datadriven::spatial_refinement ref(dimension, level, datadriven_refinement_min_support,
                                               trainingData);
      ref.refine();
    ...
    std::chrono::duration<double> grid_create_seconds = grid_create_stop - grid_create_start;
    std::cout << "grid create duration: " << grid_create_seconds.count() << "s" << std::endl;
    ...
    for (size_t i = 0; i < refinement_steps; i++) {
      ...
      if (b_coarsening && i == 0) {
        size_t old_size = alpha.getSize();
        sgpp::base::DensityBCoarseningFunctor b_coarsen_functor(b, b_coarsening_threshold);
        grid_generator.coarsen(b_coarsen_functor, b);
        alpha.resize(grid->getSize());
        std::cout << "b-based coarsening: removed " << (old_size - grid->getSize())
                  << " grid points" << std::endl;
        operation_mult = std::unique_ptr<datadriven::DensityOCLMultiPlatform::OperationDensity>(
            datadriven::createDensityOCLMultiPlatformConfigured(*grid, dimension, lambda,
                                                                configFileName));
      }
    ...
    std::cout << "density_duration_total: " << density_elapsed_seconds.count() << "s" << std::endl;
  ...
  if (write_evaluated_density_full_grid) {
    ...
    std::cout << "total density evaluation full grid grid points: " << total_grid_points
              << std::endl;
  ...
  if (reuse_knn_graph.compare("") != 0) {
    ...
    std::chrono::time_point<std::chrono::system_clock> reuse_graph_start, reuse_graph_stop;
    ...
    std::chrono::duration<double> reuse_graph_seconds = reuse_graph_stop - reuse_graph_start;
    std::cout << "loading graph (reuse) duration: " << reuse_graph_seconds.count() << "s"
              << std::endl;
  } else {
    ...
    if (knn_algorithm.compare("lsh_cuda") == 0) {
      ...
      double ops_create_graph =
          static_cast<double>(std::pow(trainingData.getNrows(), 2) * 3 * dimension) * 1E-9;
    ...
    if (variables_map.count("compare_knn_csv_file_name") > 0) {
      ...
      double acc_distance =
          knn_op.test_distance_accuracy(trainingData, neighbors_reference, graph_converted,
  ...
  auto print_knn_graph = [&trainingData, k](std::string filename, std::vector<int64_t> &graph) {
...

  {
    ...
    if (write_pruned_knn_graph) {
      std::chrono::time_point<std::chrono::system_clock> write_pruned_knn_graph_start,
          write_pruned_knn_graph_stop;
      ...
      std::cout << "write pruned knn graph duration:" << write_pruned_knn_graph_seconds.count()
                << "s" << std::endl;
    }
  }

  {
    ...
    std::cout << "find_cluster_duration_total: " << find_cluster_elapsed_seconds.count() << "s"
              << std::endl;
      ...
      double score =
          static_cast<double>(
              ...
              std::min(target_clusters,
                       std::abs(target_clusters - static_cast<int64_t>(clusters.size())))) *
          (static_cast<double>(sum_datapoints) / static_cast<double>(trainingData.getNrows()));
    ...
    if (write_cluster_map) {
      std::chrono::time_point<std::chrono::system_clock> write_cluster_map_start,
          write_cluster_map_stop;
      ...
      std::cout << "write cluster map duration:" << write_cluster_map_seconds.count() << "s"
                << std::endl;
    }
  }
