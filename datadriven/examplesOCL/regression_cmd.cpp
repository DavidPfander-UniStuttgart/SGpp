...
int main(int argc, char *argv[]) {
  ...
  description.add_options()
      ...
      ("adaptConfig.noPoints", boost::program_options::value<size_t>(&adaptConfig.noPoints_),
       "number of points to refine")(
          ...
          "refinement")("adaptConfig.use_support_refinement",
                        boost::program_options::bool_switch(&adaptConfig.use_support_refinement),
                        "use support refinement to guess an initial grid "
                        "without using any solver")(
          "adaptConfig.support_refinement_min_support",
          boost::program_options::value<int64_t>(&adaptConfig.support_refinement_min_support)
              ->default_value(1),
          ...
          boost::program_options::value<std::string>(&adaptConfig.support_refinement_ocl_config)
              ->default_value(""),
          "if set, uses this OCL config for creation of support-refined grid")(
          "adaptConfig.use_weight_support_coarsening",
          boost::program_options::bool_switch(&adaptConfig.use_weight_support_coarsening),
          "coarsen based on weighted support")(
          "adaptConfig.weight_support_coarsening_threshold",
          boost::program_options::value<double>(&adaptConfig.weight_support_coarsening_threshold)
              ->default_value(0.0),
      ...
      ("operation.type",
       ...
       "implementation type of the operation")(
          ...
          "do_dummy_ocl_init", boost::program_options::bool_switch(&do_dummy_ocl_init),
  ...
  if (do_dummy_ocl_init) {
    ...
    std::cout << "ocl dummy initalization duration: " << total_duration << std::endl;
  }
  ...
  if (variables_map.count("additionalConfig")) {
    auto parameters =
        std::make_shared<sgpp::base::OCLOperationConfiguration>(additionalConfigFileName);
